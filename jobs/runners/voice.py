...
def run_stage(job: dict, params: dict) -> dict:
    ...
    # Deduplicate by the SAME key the renderer will look up with, so a hit here
    # is a hit there by construction.
    tasks: dict[str, tuple] = {}
    for _, row in frame.iterrows():
        text = synth.normalize(row.get(script_pool.VOICEOVER_COLUMN))
        if not text or text.lower() in {"nan", "none"}:
            continue
        voice = str(row.get(script_pool.VOICE_COLUMN) or "").strip() or default_voice
        try:
            speed = float(row.get("Voiceover_Speed"))
    ...
    try:
        ...
        # A process pool can be unavailable (a locked-down container, a spawn
        # failure on Windows). Falling back to serial is slower, never wrong.
        ...
        for text, voice, speed in pending:
            try:
                ...
                logger.warning("Job %s: a voiceover failed (%s)", job_id, inner)
                record(None)

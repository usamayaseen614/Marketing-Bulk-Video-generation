...
# staged as input.mp4, input_2.mp4 … input_600.mp4 (see stage_uploads).
#
...
#
# Cheap in code, expensive in bytes: every promo is held in the Streamlit
# session for the whole browser session AND staged into the job's assets/
# folder. At 30 MB apiece, 600 promos is ~18 GB in both places — nothing on a
# c3d-standard-180 (720 GB RAM), and the staged copy is reclaimed by the
# purge-on-finish in jobs/store.py. The real cost is the upload itself: 18 GB
# through one browser tab, which Streamlit cannot resume if the tab reloads.
MAX_PROMO_VIDEOS = 600
...
def workspace_from_dir(assets: Path, work_dir: Path) -> Workspace:
    ...
    # input.mp4 first, then input_2.mp4 … input_600.mp4 in numeric order.

...
- **Disk: a job's folder is deleted the moment its output is confirmed in
  ...
  *not* be published — Drive unconfigured, an upload that failed, or one of
  ...
  retention window. To reclaim disk by hand right now, over SSH on the VM —
  the container name is generated by the COS agent (`klt-video-generator-…`),
  so resolve it by image rather than typing it:

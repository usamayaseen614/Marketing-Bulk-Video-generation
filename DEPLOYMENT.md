...
new service account has no Drive access at all until you do. If pipeline jobs
pull clips from a Drive folder, this account needs **read access to that source
folder too** — membership of the destination Shared Drive does not cover it.
...
To go back, unset the variable.

**Automatic failover.** Set `BVG_DRIVE_IMPERSONATE_FALLBACKS` to one or more
addresses (comma-separated) and the app starts on the account above and steps
to the next one on its own when that allowance runs out:

```
BVG_DRIVE_IMPERSONATE_FALLBACKS=video-uploads-3@companion-app-26947.iam.gserviceaccount.com
```

It only switches after Drive has refused a transfer for the **full retry
budget** (about eight minutes at the defaults) **and** a 2-byte test file is
refused too — so an ordinary burst of throttling is still just retried on the
same account, and the second allowance is not spent on a blip. When it does
switch it logs:

```
Drive refused the upload of yt.zip as video-uploads-2@… for the full retry
budget and refuses a 2-byte test file too — its 750 GB/24h allowance is spent.
Publishing as video-uploads-3@… from now on.
```

Every address in the list needs **both** setup steps above — the token-creator
binding *and* Content Manager on the Shared Drive. Run *Test Drive access* on
the Setup page before a big night: with fallbacks configured it now checks
every account in turn and reports them separately. An account that was never
added to the Shared Drive makes a run fail **faster** than no failover at all,
because a permission 403 is not retryable.

The position is remembered for the life of the worker process, not across a
container restart, and after 24 hours it returns to the first account on its
own — by then the allowance it moved off has refilled.
...
- **Disk: a job's folder is deleted the moment its output is confirmed in
  ...
  *not* be published — Drive unconfigured, an upload that failed, or one of
  ...
  retention window. To reclaim disk by hand right now, over SSH on the VM from
  the compose directory (`~/app`):

  ```bash
  sudo docker exec $(sudo docker compose ps -q app) \
    python -c "from jobs import store; print(store.reap_old_jobs(0))"
  ```

  On a VM deployed with `--container-image` rather than compose there is no
  `app` service to name, and the COS agent generates the container name, so
  resolve it by image instead:

  ```bash
  CID=$(sudo docker ps --format '{{.ID}} {{.Image}}' | grep bulk-video-generator | cut -d' ' -f1)
  sudo docker exec $CID python -c "from jobs import store; print(store.reap_old_jobs(0))"
  ```

  That deletes every finished job's folder immediately, ignoring the retention
  window — including jobs whose output was never published, so check the Jobs
  page for anything still holding unpublished videos before running it. It also
  prints the ids it removed. `df -h /var/lib/videogen` before and after shows
  what came back.

...
<style>
  ...
  #videoBox > img.frame { position: absolute; width: auto; height: auto; }
...
<div id="wrap">
  ...
  <div id="panel">
    ...
    <div class="hint">Drag elements to move them, drag the corner handle to resize
      (texts resize their font size), and recolor texts with the swatches. A dotted
      line appears when an element is centered on the canvas. Changed values are
...
(function () {
  ...
  function init(DATA, nonce) {
    ...
    DATA.texts.forEach(t => {
      ...
      const item = {
        x: t.cx, y: t.cy, s: 1, color: t.color,

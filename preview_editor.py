...
<style>
  ...
  /* A text's optional fit box, drawn as its own outline behind the text. It is
     ...
     target while the corner sizes the box. Visible at all times for the same
  ...
  input[type=range] { width: 80px; margin-left: 6px; vertical-align: middle;
...
<div id="wrap">
  ...
  <div id="panel">
    ...
    <div class="hint">Drag elements to move them, drag the corner handle to resize
      ...
      sliders make a text — or just its box — translucent, so the video shows
      through. Font and artistic
...
(function () {
  ...
  function init(DATA, nonce) {
    ...
    const Z = DATA.z || { video: 1, gif: 2, cta_video: 3, cta_image: 4, text: 5 };
    ...
    //     (priority promo < gifs < CTA video < CTA image < texts, as in the
    //     render). Hoisted like ctaEl so the gif box below can insert under it. ---
    let cvEl = null;
    if (DATA.cta_video) {
    ...
    if (DATA.gif) {
      ...
      gEl.className = 'el';
      gEl.id = 'gifBox';
      const gFrame = document.createElement('img');
      gFrame.className = 'frame';
      gFrame.src = g.frame;
      gFrame.draggable = false;
      gEl.appendChild(gFrame);
      gEl.style.zIndex = (Z.gif === undefined ? 2 : Z.gif);
      // A still can show one gif of the sequence and no motion at all, so say
      // so on the box rather than implying this frame is the whole layer.
      if (g.count > 1) gEl.title = 'GIF 1 of ' + g.count + ' in this video';
      // DOM order breaks z-index ties; the gif is the lowest overlay after the
      // promo video, so put it under whatever already exists.
      if (cvEl) stage.insertBefore(gEl, cvEl);
      else if (ctaEl) stage.insertBefore(gEl, ctaEl);
      else stage.appendChild(gEl);
      const gItem = {
        ...
        place() {
          ...
          gFrame.style.left = px((this.w - fw) / 2);
          gFrame.style.top = px((this.h - fh) / 2);
        },
        startResize(e) { return { w0: this.w, h0: this.h, sx: e.clientX, sy: e.clientY }; },
        resize(ctx, ev) {
          this.w = clamp(ctx.w0 + (ev.clientX - ctx.sx) / scale, __MIN_VIDEO_DIM__, DATA.canvas_w - this.x);
        ...
        endResize() { this.w = Math.round(this.w); this.h = Math.round(this.h); },
        reset() { this.x = g.x; this.y = g.y; this.w = g.w; this.h = g.h; this.place(); },
        cols() {
          return [
            ['GIF_X', g.x, Math.round(this.x)],
            ['GIF_Y', g.y, Math.round(this.y)],
            ['GIF_Width', g.w, Math.round(this.w)],
            ['GIF_Height', g.h, Math.round(this.h)],
          ];
        },
      };
      gItem.place();
      makeDraggable(gEl, gItem);
      addHandle(gEl, gItem);
      items.push(gItem);
    }
    ...
    DATA.texts.forEach(t => {
      // Opacity is CSS-only here (the mask and the baked decoration are both
      // rendered fully opaque), so the slider is live without a re-render.
      // Older payloads without the fields read as fully opaque.
      const OP = t.opacity == null ? 100 : t.opacity;
      const BOP = t.bg_opacity == null ? 100 : t.bg_opacity;
      ...
      // Optional fit box. When a text has one, the box — not the font — is what
      // the corner handle changes; the text keeps the size the server fitted and
      // re-fits on the next Preview. The wrap-and-shrink search lives in Python,
      // and a second copy of it here is exactly how the editor and the render
      // start disagreeing.
      const boxed = !!(t.box_w && t.box_h);
      let boxEl = null;
      if (boxed) {
        boxEl = document.createElement('div');
        boxEl.className = 'fitbox';
        boxEl.style.zIndex = Z.text;
        boxEl.title = t.role + ' fit box — drag the corner to resize; the text '
          + 're-fits when you preview again';
        stage.appendChild(boxEl);
      }
      const item = {
        x: t.cx, y: t.cy, s: 1, color: t.color, bg: t.bg || null,
        opacity: OP, bgOpacity: BOP,
        bw: t.box_w || 0, bh: t.box_h || 0,
        curW() { return t.w * this.s; },
        curH() { return t.h * this.s; },
        fontSize() { return Math.round(t.size * this.s); },
        center() { return { cx: this.x, cy: this.y }; },
        bounds() {
          // A boxed text is kept on canvas by its BOX: that is the space it
          // occupies, and the fitted text never leaves it.
          const w = boxed ? this.bw : this.curW();
          const h = boxed ? this.bh : this.curH();
        ...
        place() {
          ...
          if (boxEl) {
            // Same centre as the text — that is what makes adding a box to an
            // existing sheet leave the text exactly where it was.
            boxEl.style.width = px(this.bw);
            boxEl.style.height = px(this.bh);
            boxEl.style.left = px(this.x - this.bw / 2);
            boxEl.style.top = px(this.y - this.bh / 2);
          }
        ...
        applyBg() {
          ...
          // The box opacity only means something once there IS a box.
          if (this.bgOpSlider) this.bgOpSlider.disabled = !this.bg;
        },
        applyOpacity() {
          // Glyph mask and baked decoration fade together (the render fades the
          // fill, the outline ring, the glow and the drop shadow alike).
          ink.style.opacity = this.opacity / 100;
          deco.style.opacity = this.opacity / 100;
          bgBox.style.opacity = this.bgOpacity / 100;
        },
        startResize(e) {
          if (boxed) return { bw0: this.bw, bh0: this.bh, sx: e.clientX, sy: e.clientY };
        ...
        resize(ctx, ev) {
          if (boxed) {
            // The box is centred on x/y, so pulling the corner out by d has to
            // grow the box by 2d or the centre would drift as you drag.
            this.bw = clamp(ctx.bw0 + 2 * (ev.clientX - ctx.sx) / scale, 40, DATA.canvas_w);
            this.bh = clamp(ctx.bh0 + 2 * (ev.clientY - ctx.sy) / scale, 40, DATA.canvas_h);
            return;
          }
        ...
        endResize() {
          if (boxed) { this.bw = Math.round(this.bw); this.bh = Math.round(this.bh); return; }
          this.s = this.fontSize() / t.size;   // snap to a whole font size
        },
        reset() {
          this.x = t.cx; this.y = t.cy; this.s = 1; this.color = t.color;
          this.bw = t.box_w || 0; this.bh = t.box_h || 0;
          this.bg = t.bg || null;
          this.opacity = OP; this.bgOpacity = BOP;
          ink.style.backgroundColor = t.color;
          if (this.picker) this.picker.value = t.color;
          if (this.opSlider) this.opSlider.value = OP;
          if (this.bgOpSlider) this.bgOpSlider.value = BOP;
          this.applyBg();
          this.applyOpacity();
          this.place();
        },
        ...
        setOpacity(v) { this.opacity = Number(v); this.applyOpacity(); },
        setBgOpacity(v) { this.bgOpacity = Number(v); this.applyOpacity(); },
        cols() {
          // Opacities carry the '%' so the Excel cell is unambiguous: the engine
          // reads a bare 0..1 as a fraction, which would turn "1" into 100%.
          return [
            [t.role + '_X', t.cx, Math.round(this.x)],
            [t.role + '_Y', t.cy, Math.round(this.y)],
            // A boxed text derives its size from the box, so write the box back
            // rather than a size the next render would compute over anyway.
            ...(boxed
                ? [[t.role + '_Width', t.box_w, Math.round(this.bw)],
                   [t.role + '_Height', t.box_h, Math.round(this.bh)]]
                : [[t.role + '_Size', t.size, this.fontSize()]]),
            [t.role + '_Color', t.color, this.color],
            [t.role + '_Opacity', OP + '%', this.opacity + '%'],
            [t.role + '_BgColor', t.bg || '', this.bg || ''],
            [t.role + '_BgOpacity', BOP + '%', this.bgOpacity + '%'],
      ...
      // The handle goes on the BOX for a boxed text, so the corner sizes the
      // box while the text itself remains what you grab to move it.
      addHandle(boxed ? boxEl : el, item);
      items.push(item);
    });
    ...
    items.forEach(item => {
      item.cols().forEach((col, i) => {
        ...
        } else if (col[0].endsWith('_BgOpacity') || col[0].endsWith('_Opacity')) {
          // Translucency, 0-100%. The box slider stays disabled until the text
          // actually has a background box.
          const isBg = col[0].endsWith('_BgOpacity');
          const slider = document.createElement('input');
          slider.type = 'range';
          slider.min = 0; slider.max = 100; slider.step = 1;
          slider.value = parseInt(col[1], 10);
          slider.addEventListener('input', () => {
            if (isBg) item.setBgOpacity(slider.value); else item.setOpacity(slider.value);
            refresh();
          });
          if (isBg) { item.bgOpSlider = slider; slider.disabled = !item.bg; }
          else { item.opSlider = slider; }
          tdNow.appendChild(slider);
